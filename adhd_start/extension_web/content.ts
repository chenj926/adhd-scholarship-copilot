...
function highlightWord(root: HTMLElement, word: string, onClick: () => void) {
  const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT);
  const regex = new RegExp(`\\b(${word})\\b`, "gi");
  const nodesToSplit: Text[] = [];
...
function ensureSniperHud(): HTMLDivElement {
...
function showHitEffect(skill: string) {
...
function activateKeywordSniper(skills: string[]) {
...
// === Game 3: One-Question mode (lite dim version) ===
...
let oqSteps: FieldStep[] = [];
let oqIndex = 0;
let oqOverlay: HTMLDivElement | null = null;

function collectFields(root: HTMLElement): FieldStep[] {
  const inputs = Array.from(
    root.querySelectorAll("input, textarea, select")
  ) as (HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement)[];
  const result: FieldStep[] = [];
...
function ensureOqOverlay(): HTMLDivElement {
...
function applyOqDim(currentEl: HTMLElement) {
...
function updateOqOverlay() {
  if (!oqOverlay || oqSteps.length === 0) return;
  const text = document.getElementById(
    "adhd-one-question-text"
  ) as HTMLSpanElement;
  text.textContent = `Step ${oqIndex + 1}/${oqSteps.length}: ${
    oqSteps[oqIndex].label
  }`;
}

function activateOneQuestionMode() {
  oqSteps = collectFields(document.body);
  if (oqSteps.length === 0) return;
  oqIndex = 0;
  ensureOqOverlay();
...
function ensureChainBadge(): HTMLDivElement {
