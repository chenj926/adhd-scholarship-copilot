...
function renderEligibility(container, data) {
  ...
  container.innerHTML =
    ...
    (reasonsHtml
      ...
      : "") +
    (missingHtml
      ? `<div style="margin-top:4px;"><b>Missing info / unclear:</b><ul style="margin:4px 0 0 16px">${missingHtml}</ul></div>`
      : "");
}
...
async function onCheckEligibility() {
  ...
  try {
    ...
    if (!profile) {
      if (status) {
        status.textContent =
          "No profile found. Click Profile and fill in your info first.";
      }
      return;
    }
    ...
    if (out) renderEligibility(out, result);
  } catch (err) {
    console.error(err);
    if (status) {
      status.textContent = err?.message || "Could not check eligibility.";
    }
  }
}
