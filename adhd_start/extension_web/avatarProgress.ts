...
export async function loadAvatar(appId: string): Promise<AvatarState> {
  return new Promise((resolve) => {
    chrome.storage.local.get([storageKey(appId)], (data) => {
      resolve(
        (data[storageKey(appId)] as AvatarState) || { xp: 0, level: 1 }
      );
    });
  });
}
 ...
 */
export function renderAvatarHud(appId: string, state: AvatarState): void {
  ...
  (document.getElementById(
    "adhd-avatar-bar-fill"
  ) as HTMLDivElement).style.width = `${pct}%`;
  (document.getElementById(
    "adhd-avatar-label"
  ) as HTMLDivElement).textContent = `Application Strength: ${pct}% · Lv.${state.level}`;
}
