...
export async function saveChain(state) {
    return new Promise((resolve) => {
        if (!state) {
 ...
 */
export async function startChain(userId, appId) {
    const state = {
        userId,
        appId,
        chain: 0,
 ...
 */
export async function breakChain(reason) {
    const state = await loadChain();
    if (!state || state.broken)
        return;
    state.broken = true;
...
// --- UI helpers ---
function renderBadge(state, justBroken = false) {
...
function removeBadge() {
    var _a;
    (_a = document.getElementById("adhd-focus-chain")) === null || _a === void 0 ? void 0 : _a.remove();
}
