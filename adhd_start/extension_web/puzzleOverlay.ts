...
export type PuzzleResult = "success" | "fail" | "cancel";
...
function buildPuzzleUI(
  onFinish: (result: PuzzleResult) => void
): HTMLDivElement {
  ...
  doneBtn.addEventListener("click", () => {
    const result: PuzzleResult =
      failed || currentIndex < sequence.length ? "fail" : "success";
    onFinish(result);
    overlay.remove();
  });
...
function showPuzzle(): Promise<PuzzleResult> {
...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  if (msg?.type === "SHOW_PUZZLE_OVERLAY") {
