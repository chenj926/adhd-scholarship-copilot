...
function applyDim(currentEl: HTMLElement) {
...
function updateOverlay() {
  ...
  text.textContent = `Step ${currentIndex + 1}/${steps.length}: ${
    steps[currentIndex].label
  }`;
}

export function activateOneQuestionMode(root?: HTMLElement): void {
...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  if (msg?.type === "ACTIVATE_ONE_QUESTION") {
